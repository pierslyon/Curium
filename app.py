...
def _get(url, **params):
    """Small helper for requests with consistent options + error printing."""
    try:
...
def get_weather_next_8_hours():
    """
    Robust fetch:
      1) Always get current temp + weathercode for header.
      2) Separately fetch hourly forecast (48h) and slice next 8 hours.
      3) If hourly fails/empty, still show at least one slot from current.
    """
    # 1) Current weather
    cur = _get(
    ...
    if cur and isinstance(cur.get("current_weather"), dict):
        ...
        if wc is not None:
            try:
                cur_icon = icon_id_for_wmo(int(wc))
            except Exception:
                pass

    # 2) Hourly forecast (slice next 8 local hours)
    hourly = _get(
        "https://api.open-meteo.com/v1/forecast",
        latitude=BHM_LAT,
        longitude=BHM_LON,
        hourly="temperature_2m,weathercode",
        forecast_days=2,        # ensure coverage over midnight
        timezone="Europe/London",
    )
    ...
    if hourly and isinstance(hourly.get("hourly"), dict):
        ...
        for i in range(min(len(times), len(temps), len(codes))):
            try:
                # times like 'YYYY-MM-DDTHH:00' in local time
                naive = datetime.fromisoformat(times[i])
    ...
    # 3) Fallbacks so UI never empty
    if not out:
        out = [{
            "label": now_local.strftime("%H:%M"),
            "t": cur_temp,
            "icon": cur_icon
        }]
...
@app.route("/wx")
def wx():
    return jsonify(get_weather_next_8_hours())
